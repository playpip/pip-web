...
export function StatsPage() {
  ...
  const { name, avatar, roll, peakRoll, stats, rollHistory, venueRecords, tendencies } = useProfile()
  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-6xl flex-col px-6 py-8 md:px-10">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xl font-semibold lowercase tracking-tight text-muted-foreground">pip</span>
      </div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Your stats</p>
          ...
          {style.ready ? (
            <>
              ...
              <div className="mt-auto grid grid-cols-3 gap-2 pt-5">
                ...
                <Mini label="Aggression" value={`${Math.round(style.aggression * 100)}%`} sub="bet/call" />
                <Mini label="Fold to bet" value={`${Math.round(style.foldToBet * 100)}%`} sub="discipline" />
              </div>
            </>
          ) : (
          ...
          <Card>
            ...
            {rollHistory.length >= 2 ? (
              ...
              <div className="mt-3 flex h-40 items-center justify-center rounded-xl bg-foreground/[0.03]">
                <p className="text-sm text-muted-foreground">Play a few games and your story starts here.</p>
              </div>
            )}
          </Card>
...
function Card({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn('rounded-2xl border border-foreground/10 bg-foreground/[0.02] p-5', className)}>
      {children}
    </div>
  )
}
