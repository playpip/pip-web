...
test('the setting is labelled "Second opinion" and says it is usually quiet', (t) => {
  ...
  const row = source.split('function SecondOpinionSection')[1]?.split('/>')[0] ?? ''
  t.true(row.includes('<ToggleRow'), 'the section stopped rendering a toggle')
  ...
  t.false(/coaching/i.test(row), 'the word the CMO ruled out is back')
})
